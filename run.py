...
def log_metrics(mode, step, metrics, writer):
    ...
    logging.info('%s %s at step %d: %f' % (mode, metrics, step, metrics))
    writer.add_scalar("_".join([mode, metrics]), metrics, step)
...
#         kge_model = kge_model.cuda()
    ...
    if args.do_valid:
        print('do valid')
        logging.info('Evaluating on Valid Dataset...')
        metrics = kge_model.valid_step(kge_model, valid_triples, args) #这里是一个数字 MRR
    ...
    if args.evaluate_train:
        ...
        metrics = kge_model.test_step_0(kge_model, small_train_triples, args, random_sampling=True)
